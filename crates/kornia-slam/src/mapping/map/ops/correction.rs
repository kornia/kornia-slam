...
mod tests {
    ...
    /// the rotation, and a retired landmark stays retired with its geometry
    /// left as it was at retirement.
    #[test]
    fn inertial_alignment_refreshes_geometry_and_leaves_retired_landmarks_untouched() {
        let mut map = Map::new();
        ...
        let retired_geometry = {
            let dead = &map.map_points()[retired];
            (
                dead.mean_viewing_direction,
                dead.min_distance,
                dead.max_distance,
            )
        };
        ...
        // Retirement is logical: records are kept, the keyframe slot stays
        // cleared, and the correction does not refresh its geometry.
        assert!(dead.is_observed_by(0));
...
mod ba_tests {
    ...
    fn merging_an_older_snapshot_does_not_resurrect_a_retired_landmark() {
        ...
        assert!(map.map_points()[point].is_observed_by(0));
