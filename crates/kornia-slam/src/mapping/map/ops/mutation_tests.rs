...
fn assert_map_consistent(map: &Map) {
    ...
    for (mp_idx, mp) in map.map_points().iter().enumerate() {
        if mp.culled {
            // Retirement is logical (develop's behaviour): a retired landmark
            // keeps its records and derived geometry. What must hold is that no
            // keyframe still links it, which the keyframe-side pass checks.
            continue;
        }
...
fn unlinking_the_last_observation_retires_the_landmark() {
    ...
    let max_distance_before = map.map_points()[point].max_distance;
    assert_eq!(map.unlink_observation(1, 0).unwrap(), Some(point));
    assert!(map.map_points()[point].culled);
    // The unlinked record itself is gone; retirement keeps the geometry.
    assert!(map.map_points()[point].observations().is_empty());
    assert_eq!(map.map_points()[point].max_distance, max_distance_before);
    assert_map_consistent(&map);
}
